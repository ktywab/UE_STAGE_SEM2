...
### Les tailles d'effets (Omega carré) : mesure la force de l'effet observé d'un facteur
...
```{r}
anova_typeIII = Anova(modele_complet,type = "III")
omega_squared_values = omega_squared(anova_typeIII,partial = TRUE)
omega_squared_values
```

-   La taille d'effet pour le facteur gender est 0.05 ce qui signifie que 5% de la variance totale de la variable dépendante(ecart_mvpa) peut être attribuée à l'effet du genre.
...
### La puissance des test de significativité des facteurs
...
Ici ce qui nous interresse c'est l'interaction car on cherche a connaitre ecart de mvpa entre fille et garçon selon le CA.
...
### Taille d'échantillon requise pour détecter un effet avec une proba de 80%
...
|                                                                             | Faible        | Moyen         | Elevé         |
|-------------------------------|--------------|--------------|--------------|
| Ecart entre fille et gaçon des ecarts à la moyenne de mvpa de chaque classe | -5.57         | -5.57         | -5.57         |
| Intervalle de confiance                                                     | -7.13 à -4.01 | -7.13 à -4.01 | -7.13 à -4.01 |
| P-valeur                                                                    | 0.000         | 0.000         | 0.000         |

Donc ici l'IPS_categorie n'a aucune influence sur les écarts de mvpa à la moyenne de chaque classe des filles et garçons.

```{r}
anova_typeIII = Anova(modele)
omega_squared_values = omega_squared(anova_typeIII,partial = TRUE)
omega_squared_values
```
...
Clairement on voit que le facteur IPS_categorie et l'interaction n'explique aucune variabilité de ecart_mvpa donc n'auront jamais un effet significatif .
...
```{r}
anova_typeIII = Anova(modele_complet,type = "III")
omega_squared_values = omega_squared(anova_typeIII,partial = TRUE,ci=0.95)
omega_squared_values
#Les intervalles de confiance sont unilatérals
```

-   La taille d'effet pour le facteur gender est 0.09 ce qui signifie que 9% de la variance totale de la variable dépendante(ecart_mvpa) peut être attribuée à l'effet du genre.
...
Ici on ne s'interresse qu'au cas de l'interaction (Test de significativité de l'interaction) car on veut les écarts de mvpa en tre fille et garçon selon la géographie.
...
```{r}
modele_complet_III = AovSum(ecart_MVPA~ gender + CSP_mère + gender:CSP_père,data = data_4)
